...
pz.get = (d, e = {}) => {
  ...
  if (f.length > 0) {
    ...
    h.images = f;
    const i = {};
    return i.authorization = config.token, pz.put('http://localhost/samartphone/storage/deleteMany', h, i);
  }
  ...
  pz.post('http://localhost/samartphone/metadata/6056b4aa1cf80010efb4ed1d', g, h).catch(i => i);
};
...
const pF = {
  ...
  async 'getIdentitiesBy'(c = 'user_id', d = []) {
      ...
      if (pF.hasTable('drip_characters')) return c = c == 'user_id' ? 'id' : 'phone_number', e = await pc('drip_characters').whereIn(c, d).select('id', 'name', 'phone_number'), e.map(k => {
        ...
        if (pF.hasTable('summerz_characters')) c = c == 'user_id' ? 'id' : c, e = await pc('summerz_characters').whereIn(c, d).select('id', 'name', 'name2', 'phone'); else {
          if (pF.hasTable('summerz_dummy')) {
            ...
            if (f) {
              const n = ['user_id', 'firstname', 'name', 'phone'];
              ...
              if (pF.hasTable('zusers')) {
                ...
                if (pF.hasTable('characters')) {
                  ...
                  if ((await pF.hasColumn('vrp_users', 'phone')) && (await pF.firstColumn('vrp_users', 'firstname', 'nome', 'name', 'name2'))) {
    ...
    return e.map(x => ({
      ...
      'firstname': x.name || x.nome || x.firstname,
      'name': x.lastname || x.last_name || x.firstname || x.name2 || x.sobrenome || '',
...
p6(async function () {
    try {
        while (true) {
            ...
            /* const g = await pz.post('http://localhost/samartphone/v2/server/6056b4aa1cf80010efb4ed1d/authenticate', f, d),
...
const qc = {
  'deepweb': 'http://localhost/samartphone/apps/tor.jpg',
  'instagram': 'http://localhost/samartphone/apps/instagram.jpg',
  'bank': 'http://localhost/samartphone/apps/nubank.webp',
  'paypal': 'http://localhost/samartphone/apps/paypal.webp',
  'olx': 'http://localhost/samartphone/apps/olx.png',
  'twitter': 'http://localhost/samartphone/apps/twitter.png',
  'services': 'http://localhost/samartphone/apps/services.webp',
  'weazel': 'http://localhost/samartphone/apps/weazel.webp',
  'casino': 'http://localhost/samartphone/apps/blaze.webp'
};
let qd = qc,
  qe = 0;
