...
-----------------------------------------------------------------------------------------------------------------------------------------
Hope = {}
Tunnel.bindInterface("tablet",Hope)
...
-- REQUESTPOSSUIDOS
-----------------------------------------------------------------------------------------------------------------------------------------
function Hope.requestPossuidos()
	local source = source
...
-- REQUESTTAX
-----------------------------------------------------------------------------------------------------------------------------------------
function Hope.requestTax(vehName)
	local source = source
	local user_id = vRP.getUserId(source)
	if user_id then
...
-- REQUESTTRANSF
-----------------------------------------------------------------------------------------------------------------------------------------
function Hope.requestTransf(vehName)
	local source = source
	local user_id = vRP.getUserId(source)
	if user_id then
...
-- REQUESTRENTAL
-----------------------------------------------------------------------------------------------------------------------------------------
function Hope.requestRental(vehName)
	local source = source
	local user_id = vRP.getUserId(source)
	if user_id then
...
-- RENTALMONEY
-----------------------------------------------------------------------------------------------------------------------------------------
function Hope.rentalMoney(vehName)
	local source = source
	local user_id = vRP.getUserId(source)
	if user_id then
		if actived[user_id] == nil then
...
-- REQUESTBUY
-----------------------------------------------------------------------------------------------------------------------------------------
function Hope.requestBuy(vehName)
	local source = source
	local user_id = vRP.getUserId(source)
	if user_id then
		if actived[user_id] == nil then
...
-- REQUESTSELL
-----------------------------------------------------------------------------------------------------------------------------------------
function Hope.requestSell(vehName)
	local source = source
	local user_id = vRP.getUserId(source)
	if user_id then
		if actived[user_id] == nil then
			...
			if vRP.request(source,sellText) then
				...
				if vehicles[1] then
					...
					if vehType ~= "rental" then
						...
						TriggerClientEvent("itensNotify",source,{ "+","dollars",parseFormat(vehPrices),"Dólares" })
					end
				end
			end
