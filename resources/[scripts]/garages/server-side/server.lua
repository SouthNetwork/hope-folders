...
-- SERVERVEHICLE
-----------------------------------------------------------------------------------------------------------------------------------------
function Hope.serverVehicle(model,x,y,z,heading,vehPlate,nitroFuel,vehDoors,vehBody)
					...
					if Coords then
						...
						if garageLocates[garageName]["payment"] then
							if vRP.userPremium(user_id) then
								TriggerClientEvent("dynamic:closeSystem",source)
								local netExist,netVeh,mHash = Hope.serverVehicle(vehName,Coords[1],Coords[2],Coords[3],Coords[4],vehPlate,vehicle[1]["nitro"],vehicle[1]["doors"],vehicle[1]["body"])
...
RegisterCommand("car",function(source,args,rawCommand)
	...
	if user_id then
		if vRP.hasGroup(user_id,"Admin") then
			...
			local netExist,netVeh,mHash,myVeh = Hope.serverVehicle(args[1],coords["x"],coords["y"],coords["z"],heading,vehPlate,200,nil,1000)
...
-- TRYDELETE
-----------------------------------------------------------------------------------------------------------------------------------------
function Hope.tryDelete(vehNet,vehEngine,vehBody,vehFuel,vehDoors,vehWindows,vehTyres,vehPlate)
	if vehSpawn[vehPlate] then
