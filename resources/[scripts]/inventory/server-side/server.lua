...
-----------------------------------------------------------------------------------------------------------------------------------------
Cancun = {}
Tunnel.bindInterface("inventory",Cancun)
...
local stealItens = {
	...
	[33] = { ["item"] = "remapping", ["min"] = 1, ["max"] = 1, ["rand"] = 75 },
...
-- REQUESTINVENTORY
-----------------------------------------------------------------------------------------------------------------------------------------
function Cancun.requestInventory()
	local source = source
...
-- INVUPDATE
-----------------------------------------------------------------------------------------------------------------------------------------
function Cancun.invUpdate(Slot,Target,Amount)
	local source = source
	...
	if user_id and Active[user_id] == nil then
		...
		if itemType(totalName) == "Throwing" then
			...
			if checkWeapon then
				...
				if weaponStatus then
					TriggerClientEvent("itensNotify",source,{ "-",itemIndex(hashItem),1,itemName(hashItem) })
				end
			else
				...
				if vCLIENT.putWeaponHands(source,nameItem,1) then
					TriggerClientEvent("itensNotify",source,{ "+",itemIndex(totalName),1,itemName(totalName) })
		...
		if itemType(totalName) == "Munição" then
			...
			if returnWeapon then
				...
				if vRP.tryGetInventoryItem(user_id,totalName,Amount,false,Slot) then
					...
					TriggerClientEvent("itensNotify",source,{ "+",itemIndex(totalName),Amount,itemName(totalName) })
		...
		if itemType(totalName) == "Usável" or itemType(totalName) == "Animal" then
				...
				if openIdentity[user_id] then
					...
					if identity then
						...
						if numberIdentity == user_id then
							...
							TriggerClientEvent("vRP:Identity",source,{ mode = "fidentity", nome = identity["name"].." "..identity["name2"], nacionalidade = identity["locate"], porte = identity["port"], sangue = s(identity["blood"]) })
						end
					end
				end
			return end
				...
				if not vPLAYER.getHandcuff(source) then
					...
					if homeName then
						...
						if parseInt(math.random(1000)) >= 900 then
							vRP.generateItem(user_id,"brokenpick",1,false)
							vRP.removeInventoryItem(user_id,totalName,1,false)
							TriggerClientEvent("itensNotify",source,{ "-","lockpick2",1,"Lockpick de Cobre" })
						end
			...
			if nameItem == "miner" then
				if vRP.tryGetInventoryItem(user_id,totalName,1,true,Slot) then
					vRP.generateItem(user_id,"remapping",1)
			...
			if nameItem == "remapping" then
				TriggerClientEvent("inventory:Close",source)
				TriggerClientEvent("remapping:Open",source)
			return end
...
-- CHECKINVENTORY
-----------------------------------------------------------------------------------------------------------------------------------------
function Cancun.checkInventory()
	local source = source
	local user_id = vRP.getUserId(source)
	if user_id then
		if Active[user_id] ~= nil then
...
-- VERIFYWEAPON
-----------------------------------------------------------------------------------------------------------------------------------------
function Cancun.verifyWeapon(Item,Ammo)
	local source = source
	local user_id = vRP.getUserId(source)
	if user_id then
...
-- EXISTWEAPON
-----------------------------------------------------------------------------------------------------------------------------------------
function Cancun.existWeapon(Item)
	local source = source
	local user_id = vRP.getUserId(source)
	if user_id then
		local consultItem = vRP.getInventoryItemAmount(user_id,Item)
...
-- DROPWEAPONS
-----------------------------------------------------------------------------------------------------------------------------------------
function Cancun.dropWeapons(Item)
	local source = source
	local user_id = vRP.getUserId(source)
	if user_id then
		local consultItem = vRP.getInventoryItemAmount(user_id,Item)
...
-- PREVENTWEAPON
-----------------------------------------------------------------------------------------------------------------------------------------
function Cancun.preventWeapon(Item,Ammo)
	local source = source
	local user_id = vRP.getUserId(source)
	if user_id then
		local wHash = itemAmmo(Item)
...
-- REMOVETHROWABLE
-----------------------------------------------------------------------------------------------------------------------------------------
function Cancun.removeThrowable(nameItem)
	local source = source
	local user_id = vRP.getUserId(source)
	if user_id then
...
-- STEALTRUNK
-----------------------------------------------------------------------------------------------------------------------------------------
function Cancun.stealTrunk(Entity)
	local source = source
